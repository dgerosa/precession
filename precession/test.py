...
import sys,os
import __init__ as precession  # My module to compute stuff on the precession timescale
import numpy
...
def PNevolve():
    ...
    print "Initial configuration at r=ri"
    print "    (theta1,theta2,deltaphi)=(%.3f,%.3f,%.3f)" %(t1i,t2i,dpi)
    print "    (xi,J,S)=(%.3f,%.3f,%.3f)" %(xii,Ji,Si)


    print "xi J S initial", xii,Ji,Si
    
    ### ORBIT AVERAGED ## rf -> ri

    Jf,xif,Sf=precession.orbit_averaged(Ji,xii,Si,r_vals,q,S1,S2)
    
    print "xi J S orb av",Jf[-1],xif[-1],Sf[-1]

    t1f,t2f,dpf=precession.orbit_angles(t1i,t2i,dpi,r_vals,q,S1,S2)

    print "angles orb av",t1f[-1],t2f[-1],dpf[-1]

    # vectors
    Lx_fvals,Ly_fvals,Lz_fvals,S1x_fvals,S1y_fvals,S1z_fvals,S2x_fvals,S2y_fvals,S2z_fvals=precession.orbit_vectors(Ji,xii,Si,r_vals,q,S1,S2)
    
    print "vectors", Lx_fvals,Ly_fvals,Lz_fvals,S1x_fvals,S1y_fvals,S1z_fvals,S2x_fvals,S2y_fvals,S2z_fvals


    ### PRECESSION AVERAGED ## rf -> ri    
    ...
    print "kappa back", kappainf
    
    Jf_vals=precession.evolve_J_infinity(xii,kappainf,r_vals,q,S1,S2)
    
    print "from infinity", Jf_vals
    
    ### TRANSFER
    
    t1f,t2f,dpf=precession.evolve_angles(t1i,t2i,dpi,r_vals,q,S1,S2)
    print "angles prec av",t1f[-1],t2f[-1],dpf[-1]

    ### HYBRID

    t1f,t2f,dpf=precession.hybrid(xii,kappainf,r_vals,q,S1,S2,rt)
    print "angles hybrid",t1f,t2f,dpf
    
    xii,Ji,Si=precession.from_the_angles(t1f[0],t2f[0],dpf[0],q,S1,S2,ri)
    print "xi J S at ri hybrid",xii,Ji,Si
    xif,Jf,Sf=precession.from_the_angles(t1f[1],t2f[1],dpf[1],q,S1,S2,rf)
    print "xi J S at rf hybrid",xif,Jf,Sf
